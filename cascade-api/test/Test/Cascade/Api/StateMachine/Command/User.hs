...
createNotExisting =
  let
    ...
    generator model = Just do
      ...
      (usernameValidity, username) <- Gen.filter
        (flip Map.notMember usernames . view _2)
        Gen.usernameWithValidity
      (emailAddressValidity, emailAddress) <- Gen.filter
        (flip Map.notMember emailAddresses . view _2)
        Gen.emailAddressWithValidity
      (passwordValidity, password) <- Gen.passwordWithValidity
      let validity =
            [usernameValidity, emailAddressValidity, passwordValidity]
              |> all (== Valid)
              |> bool Invalid Valid
          creatable = User.RawCreatable { .. }
    ...
    require model (CreateNotExisting _ creatable) = doesNotUsernameExist
     where
      doesNotUsernameExist =
        Map.notMember (creatable ^. #username) (model ^. #user . #byUsername)
  ...
  let generator :: Model Symbolic -> Maybe (g (CreateExisting Symbolic))
      generator model | null usernames || null emailAddresses = Nothing
         ...
         where
          ...
          creatable =
            User.RawCreatable
              <$> Gen.element usernames
              <*> Gen.element emailAddresses
              <*> Gen.password Valid
      ...
      require model (CreateExisting creatable) =
        doesUsernameExist && doesEmailAddressExist
         where
          doesUsernameExist =
            Map.member (creatable ^. #username) (model ^. #user . #byUsername)
          doesEmailAddressExist = Map.member (creatable ^. #emailAddress)
                                             (model ^. #user . #byEmailAddress)

      ensure :: Model Concrete
             -> Model Concrete
             -> CreateExisting Concrete
             -> Cascade.Api.Users.CreateResponse
             -> Test ()
