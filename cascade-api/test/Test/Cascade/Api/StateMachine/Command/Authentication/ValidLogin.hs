...
import qualified Cascade.Api.Data.Authentication
                                               as Authentication
...
require model (ValidLogin Authentication.RawCredential { username, password })
  = hasUserBeenCreated && isPasswordCorrect
 where
  hasUserBeenCreated = model |> has (#user . #byUsername . ix username)
  isPasswordCorrect =
    Just password == model ^? #user . #byUsername . ix username . #password
...
update model (ValidLogin Authentication.RawCredential { username }) token =
  model |> #authToken . #byUsername . at username ?~ token
  ...
  let setCookies =
        (response ^. #responseHeaders)
          |> Seq.filter ((== hSetCookie) . view _1)
          |> fmap (parseSetCookie . view _2)

  headerAndPayload <-
    setCookies
    |> findOf folded ((== headerAndPayloadCookieName) . setCookieName)
    |> fmap setCookieValue
    |> evalMaybe
  sig <-
    setCookies
    |> findOf folded ((== signatureCookieName) . setCookieName)
    |> fmap setCookieValue
    |> evalMaybe
