...
generator model = case getByIds of
  ...
  where getByIds = model |> Model.mapUsernameTokenProjectIdAList \(username, token, id) -> GetById { .. }


require :: Model Symbolic -> GetById Symbolic -> Bool
require model GetById { id, username } = isProjectCreated && hasUserAuthToken
 where
  isProjectCreated = model |> has (#project . #byUsername . ix username . ix id)
...
execute GetById { token, id } = do
...
ensure :: Model Concrete -> Model Concrete -> GetById Concrete -> Cascade.Api.Projects.GetByIdResponse -> Test ()
  ...
  where mCreatable = before ^? #project . #byUsername . ix username . ix projectId
