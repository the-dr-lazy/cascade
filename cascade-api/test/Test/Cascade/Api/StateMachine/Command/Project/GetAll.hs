...
data GetAll (v :: Type -> Type) = GetAll
  { username :: Text
...
generator model = case model ^@.. Model.Lens.indexTokenByUsername of
  [] -> Nothing
  usernameTokenAList ->
    Just $ uncurry GetAll <$> Gen.element usernameTokenAList

require :: Model Symbolic -> GetAll Symbolic -> Bool
require model GetAll { username } =
  model |> has (#authToken . #byUsername . ix username)

execute :: MonadIO m
        => MonadTest m
        => GetAll Concrete
        -> m Cascade.Api.User.Projects.GetAllResponse
execute GetAll { token } = do
  ...
  for_ readables $ \readable -> do
    ...
    (Map.lookup id projects |> evalMaybe)
      >>= Cascade.Api.Projects.testReadableVsCreatable readable
