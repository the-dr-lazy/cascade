...
require model DeleteById { username, id } =
...
update :: Ord1 v
       => Model v
       -> DeleteById v
       -> Var Cascade.Api.Projects.DeleteByIdResponse v
       -> Model v
update model DeleteById { username, id } _ =
  model |> #project . #byUsername . ix username %~ sans id

ensure :: Model Concrete
       -> Model Concrete
       -> DeleteById Concrete
       -> Cascade.Api.Projects.DeleteByIdResponse
       -> Test ()
