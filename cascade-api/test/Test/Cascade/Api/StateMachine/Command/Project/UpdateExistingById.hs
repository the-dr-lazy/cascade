...
generator :: forall g
           . MonadGen g
          => Model Symbolic
          -> Maybe (g (UpdateById Symbolic))
generator model = case updateByIds of
  [] -> Nothing
  _  -> Just <| Gen.choice updateByIds
 where
  updateByIds :: [g (UpdateById Symbolic)]
  updateByIds = model |> Model.mapUsernameTokenProjectIdAList
    \(username, token, id) -> do
      updatable <- Gen.project
      pure UpdateById { .. }

require :: Model Symbolic -> UpdateById Symbolic -> Bool
require model UpdateById { username, id } =
  doesProjectCreated && doesUserHasAuthToken
 where
  doesProjectCreated =
...
execute input@UpdateById { token, id, updatable } = do
  ...
  response <- evalIO
    $ Cascade.Api.Projects.updateById (concrete token) (concrete id) updatable
  ensure input response

update :: Ord1 v => Model v -> UpdateById v -> Var () v -> Model v
update model UpdateById { username, id, updatable } _ =
  model
    |> (#project . #byUsername . ix username . ix id)
    %~ Cascade.Api.Projects.updateCreatable updatable

ensure :: MonadTest m
       => UpdateById Concrete
       -> Cascade.Api.Projects.UpdateByIdResponse
       -> m ()
