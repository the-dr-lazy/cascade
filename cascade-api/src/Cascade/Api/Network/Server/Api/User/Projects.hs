...
handleCreate :: Member Database.ProjectL r => Project.Creatable -> Session -> Sem r (Union CreateResponse)
handleCreate creatable =
  Session.withAuthenticated \claims -> Database.Project.create creatable (claims ^. #userId) >>= respond . Response.created

handleGetAll :: Member Database.ProjectL r => Session -> Sem r (Union GetAllResponse)
handleGetAll = Session.withAuthenticated \claims -> Database.Project.findAllByUserId (claims ^. #userId) >>= respond . Response.ok
