...
eq :: SqlEq expression a
   => Getting a (table expression) a
   -> a
   -> table expression
   -> expression Bool
optic `eq` x = \row -> view optic row == x

literal :: SqlValable (Beam.QGenExpr context backend s b)
        => Coercible a b => a -> Beam.QGenExpr context backend s b
...
type ManyToMany backend s through left right
  ...
  -> Q
       backend
       s
       ( left (Beam.QExpr backend s)
       , right (Beam.QExpr backend s)
       )

manyToMany :: ManyToMany backend s through left right
manyToMany through left right =
  Beam.manyToMany_ (database ^. through) (view left) (view right)
